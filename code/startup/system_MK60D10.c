...
/* ----------------------------------------------------------------------------
   -- SystemInit()
   ---------------------------------------------------------------------------- */
#define ClockSource_EX8MS ((uint8_t)0x07)
#define CoreClock_100M	((uint8_t)0x12)
extern void SystemClockSetup(uint8_t ClockOption, uint16_t CoreClock); //设置系统时钟
void SystemInit (void) {
  ...
  MCG->C2 = (uint8_t)0x24u;
#elif (CLOCK_SETUP == 3)
	SystemClockSetup(ClockSource_EX8MS, CoreClock_100M);
#endif /* (CLOCK_SETUP == 3) */
}
