...
/* GUI绘图的范围和起点 */
PointXY point_base;
PointXY Current_Coor;//当前坐标
uint8_t API_Mode;//1:API MODE
...
/**
  ...
  * @verify .
  */
uint8_t GUI_Set_X0_Y0_Base (const PointXY *p)
{
	uint8_t ret = FALSE;
	if (p->x < GUI_LCM_XMAX && p->y < GUI_LCM_YMAX){
		point_base.x = p->x;
		point_base.y = p->y;
		point_base.hight = p->hight;
		point_base.with = p->with;
		ret = TRUE;
	}
	return ret;
}
/**
  ...
  * @verify .
  */
void GUI_Get_X0_Y0_Base (const PointXY *p)
{
	uint8_t ret = FALSE;
	p = &point_base;
}
/**
  ...
  * @verify .
  */
uint16_t GUI_Get_Offset_X0 (void)
{
	return point_base.x;
}
/**
...
/**
  ...
  * @verify .
  */
uint16_t GUI_Get_Offset_Y0 (void)
{
	return point_base.y;
}
/**
  ...
  * @verify .
  */
void GUI_Point (int16_t x, int16_t y, TCOLOR color)
{
	if(API_Mode){
		API_GUI_Point (x, y, color);
	}else{
		API_GUI_Point (x+point_base.x, y+point_base.y, color);
	}
}
/**
  * @brief  .
  ...
  * @verify .
  */
void GUI_ReadPoint (int16_t x, int16_t y, TCOLOR *color)
{
	if(API_Mode){
		API_GUI_ReadPoint (x, y, color);
	}else{
		API_GUI_ReadPoint (x+point_base.x, y+point_base.y, color);
	}
}
/**
  * @brief  .
